...
export const getFounderPage = async (slug: string) => {
  try {
    const fd = new FormData();
    fd.append("status", "founder-page");
    fd.append("link", slug);
...
export const getLocationsPage = async () => {
  try {
    const fd = new FormData();
    fd.append("status", "locations-page");
    let request = await fetch(DOMAIN + "react/", {
      method: "POST",
      body: fd,
    });
...
export const getServicesContent = async (slug: string) => {
  const formData = new FormData();
...
export const getSeo = async (
  ...
  return response.yoast_head_json || null;
};
...
export const getSubscribeBlock = async () => {
  const formData = new FormData();
  formData.append("status", "subscribe-block");
