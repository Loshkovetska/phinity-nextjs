...
export const getLocations = async () => {
  try {
    ...
    const res = response?.sort((c: any, b: any) =>
      c.title.localeCompare(b.title)
    );
    return res || [];
  } catch (e) {
    console.log(e);
...
export const getTherapistsAll = async () => {
  const fd = new FormData();
  fd.append("status", "therapists-all");
  let request = await fetch(DOMAIN + "react/", {
    method: "POST",
    body: fd,
  });
